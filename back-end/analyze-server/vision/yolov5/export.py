...
def export_openvino(file, metadata, half, int8, data, prefix=colorstr('OpenVINO:')):
    ...
    if int8:
        ...
        def gen_dataloader(yaml_path, task='images', imgsz=640, workers=4):
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
    ...
    if int8:
        ...
        dataset = LoadImages(check_dataset(check_yaml(data))['images'], img_size=imgsz, auto=False)
...
def pipeline_coreml(model, im, file, names, y, prefix=colorstr('CoreML Pipeline:')):
    ...
    if platform.system() == 'Darwin':
        img = Image.new('RGB', (w, h))  # images(192 width, 320 height)
        # images = torch.zeros((*opt.img_size, 3)).numpy()  # images size(320,192,3) iDetection
...
def parse_opt(known=False):
    ...
    parser.add_argument('--imgsz', '--images', '--images-size', nargs='+', type=int, default=[640, 640], help='image (h, w)')
