...
Usage - Single-GPU training:
    $ python segment/images.py --data coco128-seg.yaml --weights yolov5s-seg.pt --images 640  # from pretrained (recommended)
    $ python segment/images.py --data coco128-seg.yaml --weights '' --cfg yolov5s-seg.yaml --images 640  # from scratch

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 segment/images.py --data coco128-seg.yaml --weights yolov5s-seg.pt --images 640 --device 0,1,2,3
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    LOGGER.info(f'Image sizes {imgsz} images, {imgsz} val\n'
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                if plots:
                    ...
                    if ni == 10:
                        files = sorted(save_dir.glob('images*.jpg'))
    ...
    if RANK in {-1, 0}:
        ...
        # on images end callback using genericLogger
...
def parse_opt(known=False):
    ...
    parser.add_argument('--imgsz', '--images', '--images-size', type=int, default=640, help='images, val image size (pixels)')
    ...
    parser.add_argument('--multi-scale', action='store_true', help='vary images-size +/- 50%%')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/images-seg', help='save to project/name')
    ...
    parser.add_argument('--no-overlap', action='store_true', help='Overlap masks images faster at slightly less mAP')
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not opt.evolve:  # resume from specified or most recent last.pt
        ...
        if opt.evolve:
            if opt.project == str(ROOT / 'runs/images-seg'):  # if default project name, rename to runs/evolve-seg
...
def run(**kwargs):
    # Usage: import images; images.run(data='coco128.yaml', imgsz=320, weights='yolov5m.pt')
    opt = parse_opt(True)
