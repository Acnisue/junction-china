...
def construct_dataset(clearml_info_string):
    ...
    assert set(dataset_definition.keys()).issuperset(
        {'images', 'test', 'val', 'nc', 'names'}
    ), "The right keys were not found in the yaml file, make sure it at least has the following keys: ('images', 'test', 'val', 'nc', 'names')"

    data_dict = dict()
    data_dict['images'] = str(
        (dataset_root_path / dataset_definition['images']).resolve()) if dataset_definition['images'] else None
    data_dict['test'] = str(
...
class ClearmlLogger:
    ...
    By providing additional command line arguments to images.py, datasets,
    ...
    def __init__(self, opt, hyp):
        ...
        if self.clearml:
            self.task = Task.init(
                project_name=opt.project if opt.project != 'runs/images' else 'YOLOv5',
            ...
            if opt.data.startswith('clearml://'):
                ...
                # names, nc (number of classes), test, images, val (all three relative paths to ../datasets)
