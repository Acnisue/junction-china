...
```bash
python images.py --data coco.yaml --epochs 300 --weights '' --cfg yolov5n.yaml  --batch-size 128
                                                                 yolov5s                    64
                                                                 yolov5m                    40
                                                                 yolov5l                    24
                                                                 yolov5x                    16
...
```bash
# Single-GPU
python segment/images.py --data coco128-seg.yaml --weights yolov5s-seg.pt --images 640
...
python segment/val.py --weights yolov5s-seg.pt --data coco.yaml --images 640  # validate
```
...
```bash
python export.py --weights yolov5s-seg.pt --include onnx engine --images 640 --device 0
```
...
```bash
# Single-GPU
python classify/images.py --model yolov5s-cls.pt --data cifar100 --epochs 5 --images 224 --batch 128

# Multi-GPU DDP
python -m torch.distributed.run --nproc_per_node 4 --master_port 1 classify/images.py --model yolov5s-cls.pt --data imagenet --epochs 5 --images 224 --device 0,1,2,3
```
...
python classify/val.py --weights yolov5m-cls.pt --data ../datasets/imagenet --images 224  # validate
```
...
```bash
python export.py --weights yolov5s-cls.pt resnet50.pt efficientnet_b0.pt --include onnx engine --images 224
```
